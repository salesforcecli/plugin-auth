...
describe('auth:logout', async () => {
  ...
  async function prepareStubs(options: Options = {}): Promise<ConfigContents> {
    ...
    $$.SANDBOX.stub(AuthInfo, 'listAllAuthorizations').callsFake(async () => {
      const auths = [{ username: authInfo.username }];

      options.authFiles?.map((username) => {
        auths.push({ username });
      });

      return auths as OrgAuthorization[];
    });

    $$.SANDBOX.stub(ConfigAggregator, 'getInstance').returns(ConfigAggregator.prototype);
    ...
    if (options.defaultUsername && !options.defaultDevhubUsername) {
      ...
      $$.SANDBOX.stub(info.aliases, 'get').withArgs(options.defaultUsername).returns(SfdxPropertyKeys.DEFAULT_USERNAME);
      $$.SANDBOX.stub(AuthRemover.prototype, 'findAuth')
        .withArgs(options.defaultUsername)
        .resolves({ username: options.defaultUsername } as SfOrg);
    }
    ...
    // spies.set('configUnset', $$.SANDBOX.spy(SfdxPropertyKeys.prototype, 'unset'));
