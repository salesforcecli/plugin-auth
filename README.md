...
- [`sf auth accesstoken store`](#sf-auth-accesstoken-store)
- [`sf auth device login`](#sf-auth-device-login)
- [`sf auth jwt grant`](#sf-auth-jwt-grant)
- [`sf auth list`](#sf-auth-list)
- [`sf auth logout`](#sf-auth-logout)
- [`sf auth sfdxurl store`](#sf-auth-sfdxurl-store)
- [`sf auth web login`](#sf-auth-web-login)
- [`sf force auth accesstoken store`](#sf-force-auth-accesstoken-store)
- [`sf force auth device login`](#sf-force-auth-device-login)
- [`sf force auth jwt grant`](#sf-force-auth-jwt-grant)
- [`sf force auth list`](#sf-force-auth-list)
- [`sf force auth logout`](#sf-force-auth-logout)
- [`sf force auth sfdxurl store`](#sf-force-auth-sfdxurl-store)
- [`sf force auth web login`](#sf-force-auth-web-login)
...
```
USAGE
  $ sf auth accesstoken store -r <value> [--json] [--flags-dir <value>] [-d] [-s] [-a <value>] [-p]
...
EXAMPLES
  Authorize an org on https://mycompany.my.salesforce.com; the command prompts you for the access token:
    ...
    $ sf auth accesstoken store --instance-url https://mycompany.my.salesforce.com

  Authorize the org without being prompted; you must have previously set the SF_ACCESS_TOKEN environment variable to
  the access token:

    $ sf auth accesstoken store --instance-url https://dev-hub.my.salesforce.com --no-prompt
...
```
USAGE
  $ sf auth device login [--json] [--flags-dir <value>] [-i <value>] [-r <value>] [-d] [-s] [-a <value>]
...
EXAMPLES
  Authorize an org using a device code, give the org the alias TestOrg1, and set it as your default Dev Hub org:
    ...
    $ sf auth device login --set-default-dev-hub --alias TestOrg1

  Authorize an org in which you've created a custom connected app with the specified client ID (consumer key):

    $ sf auth device login --client-id <OAuth client id>

  Authorize a sandbox org with the specified instance URL:

    $ sf auth device login --instance-url https://MyDomainName--SandboxName.sandbox.my.salesforce.com
...
```
USAGE
  $ sf auth jwt grant -o <value> -f <value> -i <value> [--json] [--flags-dir <value>] [-r <value>] [-d] [-s] [-a
    <value>]
...
EXAMPLES
  ...
  key (client id) 04580y4051234051.
    ...
    $ sf auth jwt grant --username jdoe@example.org --jwt-key-file /Users/jdoe/JWT/server.key --client-id \
      04580y4051234051
...
```
USAGE
  $ sf auth list [--json] [--flags-dir <value>]
...
```
USAGE
  $ sf auth logout [--json] [--flags-dir <value>] [-a | -o <value>] [-p]
...
EXAMPLES
  ...
  Log out of the org with username me@my.org:
    ...
    $ sf auth logout --target-org me@my.org
  ...
  Logout of the org with alias my-scratch and don't prompt for confirmation:
    ...
    $ sf auth logout --target-org my-scratch --no-prompt
...
```
USAGE
  $ sf auth sfdxurl store [--json] [--flags-dir <value>] [-f <value>] [-u <value>] [-d] [-s] [-a <value>]
...
DESCRIPTION
  ...
  $ sf auth sfdxurl store --sfdx-url-file authFile.json
...
EXAMPLES
  Authorize an org using the SFDX authorization URL in the files/authFile.json file:
    ...
    $ sf auth sfdxurl store --sfdx-url-file files/authFile.json

  Similar to previous example, but set the org as your default and give it an alias MyDefaultOrg:

    $ sf auth sfdxurl store --sfdx-url-file files/authFile.json --set-default --alias MyDefaultOrg

  Pipe the SFDX authorization URL from stdin:

   $ echo url | sf auth sfdxurl store --sfdx-url-stdin
```
...
```
USAGE
  $ sf auth web login [--json] [--flags-dir <value>] [-b chrome|edge|firefox] [-i <value>] [-r <value>] [-d] [-s]
    [-a <value>]
...
EXAMPLES
  ...
  scratch org:
    ...
    $ sf auth web login --set-default-dev-hub --alias dev-hub

  Log in to a sandbox and set it as your default org:

    $ sf auth web login --instance-url https://MyDomainName--SandboxName.sandbox.my.salesforce.com --set-default
...
```
USAGE
  $ sf force auth accesstoken store -r <value> [--json] [--flags-dir <value>] [-d] [-s] [-a <value>] [-p]
...
EXAMPLES
  Authorize an org on https://mycompany.my.salesforce.com; the command prompts you for the access token:
    ...
    $ sf force auth accesstoken store --instance-url https://mycompany.my.salesforce.com

  Authorize the org without being prompted; you must have previously set the SF_ACCESS_TOKEN environment variable to
  the access token:

    $ sf force auth accesstoken store --instance-url https://dev-hub.my.salesforce.com --no-prompt
...
```
USAGE
  $ sf force auth device login [--json] [--flags-dir <value>] [-i <value>] [-r <value>] [-d] [-s] [-a <value>]
...
EXAMPLES
  Authorize an org using a device code, give the org the alias TestOrg1, and set it as your default Dev Hub org:
    ...
    $ sf force auth device login --set-default-dev-hub --alias TestOrg1

  Authorize an org in which you've created a custom connected app with the specified client ID (consumer key):

    $ sf force auth device login --client-id <OAuth client id>

  Authorize a sandbox org with the specified instance URL:

    $ sf force auth device login --instance-url https://MyDomainName--SandboxName.sandbox.my.salesforce.com
...
```
USAGE
  $ sf force auth jwt grant -o <value> -f <value> -i <value> [--json] [--flags-dir <value>] [-r <value>] [-d] [-s] [-a
    <value>]
...
EXAMPLES
  ...
  key (client id) 04580y4051234051.
    ...
    $ sf force auth jwt grant --username jdoe@example.org --jwt-key-file /Users/jdoe/JWT/server.key --client-id \
      04580y4051234051
...
```
USAGE
  $ sf force auth list [--json] [--flags-dir <value>]
...
```
USAGE
  $ sf force auth logout [--json] [--flags-dir <value>] [-a | -o <value>] [-p]
...
EXAMPLES
  ...
  Log out of the org with username me@my.org:
    ...
    $ sf force auth logout --target-org me@my.org

  Log out of all orgs after confirmation:

    $ sf force auth logout --all

  Logout of the org with alias my-scratch and don't prompt for confirmation:

    $ sf force auth logout --target-org my-scratch --no-prompt
...
```
USAGE
  $ sf force auth sfdxurl store [--json] [--flags-dir <value>] [-f <value>] [-u <value>] [-d] [-s] [-a <value>]
...
EXAMPLES
  Authorize an org using the SFDX authorization URL in the files/authFile.json file:
    ...
    $ sf force auth sfdxurl store --sfdx-url-file files/authFile.json

  Similar to previous example, but set the org as your default and give it an alias MyDefaultOrg:

    $ sf force auth sfdxurl store --sfdx-url-file files/authFile.json --set-default --alias MyDefaultOrg

  Pipe the SFDX authorization URL from stdin:

   $ echo url | sf force auth sfdxurl store --sfdx-url-stdin
```
...
```
USAGE
  $ sf force auth web login [--json] [--flags-dir <value>] [-b chrome|edge|firefox] [-i <value>] [-r <value>] [-d] [-s]
    [-a <value>]
...
EXAMPLES
  ...
  scratch org:
    ...
    $ sf force auth web login --set-default-dev-hub --alias dev-hub

  Log in to a sandbox and set it as your default org:

    $ sf force auth web login --instance-url https://MyDomainName--SandboxName.sandbox.my.salesforce.com \
      --set-default
...
_See code: [src/commands/org/list/auth.ts](https://github.com/salesforcecli/plugin-auth/blob/v3.4.0/src/commands/org/list/auth.ts)_
...
_See code: [src/commands/org/login/access-token.ts](https://github.com/salesforcecli/plugin-auth/blob/v3.4.0/src/commands/org/login/access-token.ts)_
...
_See code: [src/commands/org/login/device.ts](https://github.com/salesforcecli/plugin-auth/blob/v3.4.0/src/commands/org/login/device.ts)_
...
_See code: [src/commands/org/login/jwt.ts](https://github.com/salesforcecli/plugin-auth/blob/v3.4.0/src/commands/org/login/jwt.ts)_
...
_See code: [src/commands/org/login/sfdx-url.ts](https://github.com/salesforcecli/plugin-auth/blob/v3.4.0/src/commands/org/login/sfdx-url.ts)_
...
_See code: [src/commands/org/login/web.ts](https://github.com/salesforcecli/plugin-auth/blob/v3.4.0/src/commands/org/login/web.ts)_
...
_See code: [src/commands/org/logout.ts](https://github.com/salesforcecli/plugin-auth/blob/v3.4.0/src/commands/org/logout.ts)_
